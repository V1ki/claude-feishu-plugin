...
2. Add `<senderId>` to `allowFrom` (dedupe). Sender IDs are Feishu open_ids
   (e.g. `ou_xxx`).
3. Write back.
...
Note: Feishu group chat IDs look like `oc_xxx`.
...
- `ackReaction`: string (Feishu emoji type like "THUMBSUP") or `""` to disable
...
- Sender IDs are Feishu open_ids (e.g. `ou_xxx`). Don't validate format.
